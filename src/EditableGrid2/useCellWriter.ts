...
/**
 * グリッドの操作（セル編集の確定・貼り付け・Deleteキーによるクリア）による書き込みを行う。
 *
 * 書き込みはすべて commitWrites を経由し、列定義の setText で新しい行オブジェクトを作ってから、
 * 1回の操作につき1回だけ onRowsChange を呼ぶ。
 * React Hook Form の setValue のように1回ごとのコストが高い反映先でも、
 * 呼び出し回数がセルの数ではなく行の数で済むようにするため。
 */
export const useCellWriter = <TRow,>(
   ...
   */
  const commitWrites = (writes: CellTextWrite[]) => {
